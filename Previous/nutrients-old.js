...
d3.json(url).then((jsonData) => {
    d3.csv(csv).then((daily) => {
        ...
        //   fruits.push(jsonData[i].description)
        // }
        // console.log(fruits)
        ...
        var i = 3
        var j = 0
        summary = jsonData.filter(d => d.description == selectedOption)
        console.log("summary", summary)
        
        // var protein = summary.map(row => row.foodNutrients[0].nutrient.name)
        // console.log(protein)

        // var amount = summary.map(row => row.foodNutrients[0].amount)
        // console.log(amount)
        ...
        for (var j=0; j<summary.map(row => row.foodPortions)[0].length; j++) {
          ...
          if (String(portDesc).includes("small")){
        ...
        // var fruitPortDesc = portion.map(row => row.foodPortions[j].portionDescription)
        // console.log(fruitPortDesc)
        // var fruitPortWeight = portion.map(row => row.foodPortions[j].gramWeight)
        // console.log(fruitPortWeight)

        // Fruit name
        var fruitName = summary.map(row => row.description)
        // Protein
        var proteinAmt = summary.map(row => row.foodNutrients[0].amount)
        ...
        document.getElementById("Anchor").innerHTML = `Fruit: ${fruitName}`
        document.getElementById("Anchor_Protein").innerHTML = `Protein (${proteinUnit}): ${proteinAmt}`
